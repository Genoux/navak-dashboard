      ...
      <div
        ...
        <form class="pt-0 pb-0 grid md:grid-cols-2 gap-6 w-full sm:grid-cols-2 "
          ...
          <div class="relative z-50 overflow-visible">
            ...
            <div class=" border border-opacity-20 border-white mt-3 mb-3 h-5 rounded-sm" v-model="defaultValue.rgb" @click="showColorPicker = !showColorPicker" v-bind:style="'background-color: rgba(' + formatColor() + ')'"></div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      formatColor(){
        this.defaultValue.rgb = `${this.colors.rgba.r}, ${this.colors.rgba.g}, ${this.colors.rgba.b}`;
            ...
            (error) => {
              if (error) {
                this.openNotification('top-center', 'danger', '💀 Something want wrong, please try again',
                  'It might be the data passing to the server, please check the console');
                this.loading = false;
              } else {
