<template name="areas">
  <div>
    <div class="flex bg-dark border-b border-white border-opacity-20 h-14 pl-5 pr-5">
      ...
      <div
        @click="openDialog()"
        class="flex-end ml-auto self-center border border-white hover:opacity-60 cursor-pointer focus:bg-white p-1"
      >
    ...
    <div
      v-if="areas.length > 0"
      class="p-4 grid gap-4 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-4"
    >
    ...
    >
      ...
      <div
        class="bg-black border border-white border-opacity-25 sm:w-3/4 md:w-3/4 lg:w-3/6 pt-5 pb-8 px-10 shadow-lg z-50"
      >
        <div
        ...
        <form
          class="pt-0 pb-0 grid md:grid-cols-2 gap-6 w-full sm:grid-cols-2 lg:grid-cols-3"
          v-bind:class="{ 'opacity-10': loading, 'pointer-events-none': loading }"
        >
        ...
        >
          <button
            @click="createNewPosition(defaultValue)"
            class="py-2 px-6 flex text-center drop-shadow-lg bg-green text-white hover:opacity-90"
          >
            <mdicon class="m-auto" name="CheckBold"></mdicon>
          </button>
          <button
            @click="getPosition($event)"
            class="py-2 px-6 flex text-center drop-shadow-lg bg-blue text-white hover:opacity-90"
          >
  ...
  computed: {
    computed_items: function () {
      ...
      return this.areas.filter(function (item) {
        console.log("🚀 ~ file: list.vue ~ line 109 ~ item", item);
        let filtered = true
        if (filtered) {
