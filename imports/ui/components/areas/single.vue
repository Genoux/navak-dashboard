<template>
	<div>
		<div class="hover:border-current duration-75 ease-in bg-black text-white border rounded-sm border-white border-opacity-20">
		...
		<div v-if="activeDialog" class="fixed w-full h-full top-0 left-0 flex z-50 items-center justify-center overflow-y-scroll">
			...
			<div class="bg-black border border-white border-opacity-25 sm:w-3/4 md:w-3/4 lg:w-3/6 pt-5 pb-8 px-10 rounded-lg shadow-lg z-50">
				...
				<div class="flex w-full gap-2 pt-2 mt-6" v-bind:class="{'opacity-10': loading, 'pointer-events-none': loading}">
					...
					<button @click="deletePosition($event)" class="py-2 px-6 flex ml-auto text-center drop-shadow-lg bg-red-500 rounded-lg text-white hover:opacity-90">
