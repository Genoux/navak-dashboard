<template>
	<div>
		<div class="hover:border-current rounded-md duration-75 ease-in bg-black text-white border border-white border-opacity-20">
			<div class="flex flex-grow px-4 py-4 items-center gap-2 border-b border-white border-opacity-20">
				...
				<div class="flex">
					<div @click="openDialog(area)" class="rounded-sm bg-black border ml-2 p-1 hover:opacity-60 cursor-pointer focus:bg-white focus:text-blue focus:outline-none">
				...
				<form class=" pt-0 pb-0 grid md:grid-cols-2 gap-6 w-full sm:grid-cols-2 lg:grid-cols-3 grid-cols-2" v-bind:class="{'opacity-10': loading, 'pointer-events-none': loading}">
					<div :key="i" v-for="(value, key, i) in filteredAreas">
						...
						<input
							...
							class="w-full px-4 py-2 text-white bg-dark border border-white border-opacity-25 focus:outline-none focus:border-opacity-60 rounded-md"
						/>
					</div>
          <div class="h-full">
            <p class="text-sm text-white pb-2"> Snap Tool </p>
            <v-dropdown style="height: 42px;" CustomClass="w-full px-4 py-2 h-full rounded-md" :setAll="false" :selection="snapList" :default="snapList[0].group" @snapSelection="snapSelection($event)"></v-dropdown>
          </div>
				</form>
				<div class="flex w-full gap-3 pt-2 mt-6" v-bind:class="{'opacity-10': loading, 'pointer-events-none': loading}">
					<button @click="updatePosition(selectedArea, $event)" class="py-2 px-10 flex text-center drop-shadow-lg bg-green text-white hover:opacity-90 rounded-sm">
						<mdicon class="m-auto" name="CheckBold"></mdicon>
					</button>
					<button @click="getPosition($event)" class="py-2 px-6 flex text-center drop-shadow-lg bg-blue text-white hover:opacity-90 rounded-sm">
						<mdicon class="m-auto" name="Camera"></mdicon>
					</button>
					<button @click="deletePosition($event)" class="py-2 px-6 flex ml-auto text-center drop-shadow-lg bg-red-500 text-white hover:opacity-90 rounded-sm">
